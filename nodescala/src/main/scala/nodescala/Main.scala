...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val userInterrupted: Future[String] = Future.userInput("Tell me...") continueWith { f => "You entered... " + f.now }
    userInterrupted.onSuccess { case s => println(s) }

    // TO IMPLEMENT
    // 3. create a future that completes after 20 seconds
    //    and continues with a `"Server timeout!"` message
    val timeOut: Future[String] = Future.delay(20 seconds) continueWith { _ => "ServerTimeout!" }
